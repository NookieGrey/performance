...
const calculate = setComponentVisible => (id, phase, actualDuration) => {
    ...
    if (data.length < 100) {
        ...
        setTimeout(() => setComponentVisible(true), 50);
...
function Generator() {
    ...
    const [componentVisible, setComponentVisible] = useState({value: 0});
    ...
    return (
        <>
            ...
            {componentVisible && (
                <Profiler
                    id={Component.name}
                    onRender={calculate(setComponentVisible)}
                >
                    <Component
                        nestingLvl={10}
                    />
